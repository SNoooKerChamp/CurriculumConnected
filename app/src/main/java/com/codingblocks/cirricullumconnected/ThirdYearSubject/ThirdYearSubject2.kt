...
import kotlinx.android.synthetic.main.activity_productionand_om.*

class ThirdYearSubject2 : AppCompatActivity() {
    var teacherlist = arrayOf("Dr Girish Kumar", "Dr Saurabh Agarwal", "Dr Pravin Kumar")
    var fragmentmanager: FragmentManager? = null
    var averagelist = arrayOf("0.0", "0.0", "0.0")

    override fun onBackPressed() {
        startActivity(Intent(this@ThirdYearSubject2, YearTabbedActivity::class.java))
        super.onBackPressed()
    }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_productionand_om)

        fragmentmanager = supportFragmentManager

        var a = Adapter()
        lv_thirdyearsubject2.adapter = a
    ...
    inner class Adapter : BaseAdapter() {
        ...
        override fun getView(position: Int, convertview: View?, parent: ViewGroup): View? {
            ...
            Glide.with(this@ThirdYearSubject2).load("http://kbfacilitators.com.au/wp-content/uploads/2016/03/PDF-Logo.png")
                    .into(ibpdf)
            ...
            tvratetheapp?.setOnClickListener {
                ...
                fragmenttransaction?.add(R.id.Containerthirdyear2, Ratingfragment.newInstance(teacherlist.get(position), "Production and Operations"))?.commit()
            ...
            ibpdf?.setOnClickListener {
                when (position) {
                    0 -> {
                        val i1 = Intent(this@ThirdYearSubject2, SingleSubjectnotes::class.java)
                        i1.putExtra("pdf", "http://unec.edu.az/application/uploads/2014/12/pdf-sample.pdf")
                        startActivity(i1)
                    }
                    1 -> {
                        val i2 = Intent(this@ThirdYearSubject2, SingleSubjectnotes::class.java)
                        i2.putExtra("pdf", "http://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf")
                        startActivity(i2)
                    }
                    2 -> {
                        val i3 = Intent(this@ThirdYearSubject2, SingleSubjectnotes::class.java)
            ...
            teachername?.setOnClickListener {
                // need to change the url for teacher resume.
                when (position) {
                    0 -> {
                        val i1 = Intent(this@ThirdYearSubject2, teacherprofile::class.java)
                        i1.putExtra("url", "http://www.dtu.ac.in/Web/Departments/Mechanical/faculty/vjeganathan.php")
                        startActivity(i1)
                    }
                    1 -> {
                        val i2 = Intent(this@ThirdYearSubject2, teacherprofile::class.java)
                        i2.putExtra("url", "http://www.dtu.ac.in/Web/Departments/Mechanical/faculty/pkjain.php")
                        startActivity(i2)
                    }

                    2 -> Toast.makeText(this@ThirdYearSubject2, "Sorry!" + "Information of Guest Faculty is currently not available.", Toast.LENGTH_LONG).show()
                }
            }
